...
describe('DateToStringParser', () => {
  ...
  describe('parse - ISO format', () => {
    test('test_parse_WHEN_isoFormatWithValidDate_THEN_returnsIsoString', () => {
      ...
      const result = DateToStringParser.parse(date, 'iso');
    ...
    test('test_parse_WHEN_isoFormatWithInvalidDate_THEN_throwsSaltoolsError', () => {
      ...
      expect(() => DateToStringParser.parse(invalidDate, 'iso')).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(invalidDate, 'iso')).toThrow('date deve ser uma data válida');
    });
  });
  ...
  describe('parse - formats with separators', () => {
    test('test_parse_WHEN_ddSlashMmSlashYyyy_THEN_returnsCorrectString', () => {
      ...
      const result = DateToStringParser.parse(date, 'dd/mm/yyyy');
      expect(result).toBe('15/03/2024');
    });

    test('test_parse_WHEN_dSlashMSlashYy_THEN_returnsCorrectString', () => {
      const date = new Date(2024, 2, 5);
      const result = DateToStringParser.parse(date, 'd/m/yy');
  ...
  describe('parse - validation errors', () => {
    test('test_parse_WHEN_dateIsNotDateInstance_THEN_throwsSaltoolsError', () => {
      expect(() => DateToStringParser.parse('2024-03-15', 'dd/mm/yyyy')).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse('2024-03-15', 'dd/mm/yyyy')).toThrow('date deve ser uma instância de Date');
    });

    test('test_parse_WHEN_dateIsNull_THEN_throwsSaltoolsError', () => {
      expect(() => DateToStringParser.parse(null, 'dd/mm/yyyy')).toThrow(SaltoolsError);
    });

    test('test_parse_WHEN_dateIsUndefined_THEN_throwsSaltoolsError', () => {
      expect(() => DateToStringParser.parse(undefined, 'dd/mm/yyyy')).toThrow(SaltoolsError);
    });

    test('test_parse_WHEN_formatIsNotString_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 123)).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(date, 123)).toThrow('format deve ser string');
    });

    test('test_parse_WHEN_formatIsNull_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, null)).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(date, null)).toThrow('format é obrigatório');
    });

    test('test_parse_WHEN_invalidFormatTooManyD_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'ddd/mm/yyyy')).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(date, 'ddd/mm/yyyy')).toThrow('format deve conter 1 ou 2 "d", 1 ou 2 "m" e 2 ou 4 "y"');
    });

    test('test_parse_WHEN_invalidFormatTooManyM_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'dd/mmm/yyyy')).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(date, 'dd/mmm/yyyy')).toThrow('format deve conter 1 ou 2 "d", 1 ou 2 "m" e 2 ou 4 "y"');
    });

    test('test_parse_WHEN_invalidFormatWrongYearCount_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'dd/mm/yyyyy')).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(date, 'dd/mm/yyyyy')).toThrow('format deve conter 1 ou 2 "d", 1 ou 2 "m" e 2 ou 4 "y"');
    });

    test('test_parse_WHEN_inconsistentFormatSeparators_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'dd/mm-yyyy')).toThrow(SaltoolsError);
      expect(() => DateToStringParser.parse(date, 'dd/mm-yyyy')).toThrow('Formato contém separadores inconsistentes');
    });

    test('test_parse_WHEN_formatWithoutD_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'mm/yyyy')).toThrow(SaltoolsError);
    });

    test('test_parse_WHEN_formatWithoutM_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'dd/yyyy')).toThrow(SaltoolsError);
    });

    test('test_parse_WHEN_formatWithoutY_THEN_throwsSaltoolsError', () => {
      const date = new Date(2024, 2, 15);
      expect(() => DateToStringParser.parse(date, 'dd/mm')).toThrow(SaltoolsError);
    });
  });
  ...
  describe('parse - cache optimization', () => {
    test('test_parse_WHEN_sameFormatUsedMultipleTimes_THEN_usesCache', () => {
      ...
      const result1 = DateToStringParser.parse(date1, 'dd/mm/yyyy');
      const result2 = DateToStringParser.parse(date2, 'dd/mm/yyyy');
      const result3 = DateToStringParser.parse(date3, 'dd/mm/yyyy');
    ...
    test('test_parse_WHEN_differentFormatUsed_THEN_updatesCache', () => {
      const date = new Date(2024, 2, 15);
      DateToStringParser.parse(date, 'dd/mm/yyyy');
      const result = DateToStringParser.parse(date, 'ddmmyyyy');
