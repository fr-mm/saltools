...
import NumberParser from 'src/commands/parse/parse-number.js';
import SaltoolsError from 'src/errors/saltools-error.js';

describe('NumberParser', () => {
  describe('number', () => {
    test('test_number_WHEN_validNumberString_THEN_returnsNumber', () => {
      const result = NumberParser.parseNumber('123');
    ...
    test('test_number_WHEN_decimalString_THEN_returnsDecimalNumber', () => {
      const result = NumberParser.parseNumber('123.456');
    ...
    test('test_number_WHEN_zeroStringWithAllowZeroTrue_THEN_returnsZero', () => {
      const result = NumberParser.parseNumber('0', { allowZero: true });
    ...
    test('test_number_WHEN_zeroStringWithAllowZeroFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber('0', { allowZero: false });
    ...
    test('test_number_WHEN_negativeStringWithAllowNegativeTrue_THEN_returnsNegativeNumber', () => {
      const result = NumberParser.parseNumber('-123', { allowNegative: true });
      expect(result).toBe(-123);
    });
    ...
    test('test_number_WHEN_negativeStringWithAllowNegativeFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber('-123', { allowNegative: false });
      }).toThrow(SaltoolsError);
    });
    ...
    test('test_number_WHEN_negativeZeroWithAllowNegativeFalseAndAllowZeroFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber(-0, { allowNegative: false, allowZero: false });
      }).toThrow(SaltoolsError);
    });

    test('test_number_WHEN_zeroWithDefaultOptions_THEN_returnsZero', () => {
      const result = NumberParser.parseNumber(0);
      expect(result).toBe(0);
    });
    ...
    test('test_number_WHEN_emptyStringWithAllowEmptyTrue_THEN_returnsZero', () => {
      const result = NumberParser.parseNumber('', { allowEmptyString: true, allowZero: true });
      expect(result).toBe(0);
    });

    test('test_number_WHEN_emptyStringWithAllowEmptyFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber('', { allowEmptyString: false });
      }).toThrow(SaltoolsError);
    });

    test('test_number_WHEN_nullWithAllowNullTrue_THEN_returnsZero', () => {
      const result = NumberParser.parseNumber(null, { allowNull: true, allowZero: true });
      expect(result).toBe(0);
    });

    test('test_number_WHEN_nullWithAllowNullFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber(null, { allowNull: false });
      }).toThrow(SaltoolsError);
    });

    test('test_number_WHEN_undefined_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber(undefined);
      }).toThrow(SaltoolsError);
    });

    test('test_number_WHEN_boolean_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber(true);
      }).toThrow(SaltoolsError);
      expect(() => {
        NumberParser.parseNumber(false);
      }).toThrow(SaltoolsError);
    });

    test('test_number_WHEN_invalidString_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber('abc');
      }).toThrow(SaltoolsError);
    });

    test('test_number_WHEN_array_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseNumber([1, 2, 3]);
      }).toThrow(SaltoolsError);
    });
  ...
  describe('integer function', () => {
    test('test_integer_WHEN_validIntegerString_THEN_returnsInteger', () => {
      const result = NumberParser.parseInteger('123');
      expect(result).toBe(123);
    });
    ...
    test('test_integer_WHEN_zeroStringWithAllowZeroTrue_THEN_returnsZero', () => {
      const result = NumberParser.parseInteger('0', { allowZero: true });
      expect(result).toBe(0);
    });
    ...
    test('test_integer_WHEN_zeroStringWithAllowZeroFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseInteger('0', { allowZero: false });
      }).toThrow(SaltoolsError);
    });
    ...
    test('test_integer_WHEN_negativeIntegerStringWithAllowNegativeTrue_THEN_returnsNegativeInteger', () => {
      const result = NumberParser.parseInteger('-123', { allowNegative: true });
      expect(result).toBe(-123);
    });
    ...
    test('test_integer_WHEN_negativeIntegerStringWithAllowNegativeFalse_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseInteger('-123', { allowNegative: false });
      }).toThrow(SaltoolsError);
    });
    ...
    test('test_integer_WHEN_decimalString_THEN_throwsError', () => {
      expect(() => {
        NumberParser.parseInteger('123.456');
      }).toThrow(SaltoolsError);
    });
  ...
  describe('throwError option', () => {
    test('test_number_WHEN_invalidInputAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseNumber('abc', { throwError: false });
      expect(result).toBeNull();
    });

    test('test_number_WHEN_nullNotAllowedAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseNumber(null, { allowNull: false, throwError: false });
      expect(result).toBeNull();
    });

    test('test_integer_WHEN_decimalAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseInteger('1.2', { throwError: false });
      expect(result).toBeNull();
    });

    test('test_integer_WHEN_nullNotAllowedAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseInteger(null, { allowNull: false, throwError: false });
      expect(result).toBeNull();
    });

    test('test_number_WHEN_zeroNotAllowedAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseNumber(0, { allowZero: false, throwError: false });
      expect(result).toBeNull();
    });

    test('test_number_WHEN_negativeNotAllowedAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseNumber(-123, { allowNegative: false, throwError: false });
      expect(result).toBeNull();
    });

    test('test_integer_WHEN_zeroNotAllowedAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseInteger(0, { allowZero: false, throwError: false });
      expect(result).toBeNull();
    });

    test('test_integer_WHEN_negativeNotAllowedAndThrowErrorFalse_THEN_returnsNull', () => {
      const result = NumberParser.parseInteger(-123, { allowNegative: false, throwError: false });
      expect(result).toBeNull();
    });
  });
});
